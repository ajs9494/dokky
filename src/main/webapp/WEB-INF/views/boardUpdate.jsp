...
<body>
	...
	<form action="<c:url value="${whichBoard eq 'free' ? '/freeBoard/update/' : '/questionBoard/update/'}${board.bno}${searchCondition.queryString}" />" method="post"
		id="container" onsubmit="return formCheck(this)"><input type="hidden" name="_method" value="PATCH"/>
		<div id=title-cnt>
			<div>제목</div>
			<input type="text" id="title" name="title" placeholder="제목을 입력해주세요." value="${board.title}"></input>
		</div>
		<div id="editor"></div>
