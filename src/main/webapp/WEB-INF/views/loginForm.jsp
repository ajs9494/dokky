...
<html lang="en">
  ...
  <body>
    <div class="container">
      ...
      <form id="login-form" action="<c:url value='/login/login' />" method="post" onsubmit="return formCheck(this)">
        <div id="msg">${URLDecoder.decode(param.msg)}</div>
        <label for="id">아이디</label>
        <input type="text" id="id" name="id" value="${cookie.id.value}" placeholder="아이디를 입력해주세요" autofocus />
        <label for="pwd">비밀번호</label>
        <input type="password" id="pwd" name="pwd" placeholder="비밀번호를 입력해주세요" />
        <div class="checkbox-container">
        <label for="rememberId">아이디 기억</label><input type="checkbox" id="rememberId" name="rememberId" ${empty cookie.id.value ? "" : "checked"} />
        </div>
        <button type="submit" id="login-btn">로그인</button>
        <div id="register"><a href="<c:url value='/register/add' />">회원가입</a></div>
