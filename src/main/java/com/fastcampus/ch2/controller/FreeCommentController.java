...
public class FreeCommentController {
	...
	public ResponseEntity<List<CommentDto>> writeComment(@PathVariable Integer bno, @RequestBody CommentDto comment, HttpServletRequest request) {
		try {
			...
			// 댓글 저장 실패시 에러 상태코드
			if(freeCommentService.writeComment(comment)!=1) return new ResponseEntity<List<CommentDto>>(HttpStatus.BAD_REQUEST);
			// 댓글 저장 성공시 성공 상태코드와 댓글리스트
			List<CommentDto> list = freeCommentService.getComments(bno);
	...
	public ResponseEntity<List<CommentDto>> removeComment(@PathVariable Integer cno, Integer pcno, String writer, Integer bno, HttpServletRequest request) {
		try {
			...
			if(cno.equals(pcno)) {
				List<CommentDto> repCheckList = freeCommentService.getComments(bno);
				for (CommentDto c : repCheckList) {
					if(c.getPcno().equals(cno)) {
						if(freeCommentService.removeComment(c.getCno(), c.getWriter())!=1) return new ResponseEntity<List<CommentDto>>(HttpStatus.BAD_REQUEST);
					}
				}
			// 답글인경우 해당답글만 지운다
			} else {
				// 댓글 삭제 실패시 에러 상태코드
				if(freeCommentService.removeComment(cno, writer)!=1) return new ResponseEntity<List<CommentDto>>(HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity<List<CommentDto>> updateComment(@RequestBody CommentDto comment, HttpServletRequest request) {
		try {
			...
			// 댓글 수정 실패시 에러 상태코드
			if(freeCommentService.updateComment(comment)!=1) return new ResponseEntity<List<CommentDto>>(HttpStatus.BAD_REQUEST);
			// 댓글 수정 성공시 성공 상태코드와 댓글리스트
			List<CommentDto> list = freeCommentService.getComments(comment.getBno());
			return new ResponseEntity<List<CommentDto>>(list, HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<List<CommentDto>>(HttpStatus.BAD_REQUEST);
